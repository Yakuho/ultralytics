...
def verify_image_label(args: tuple, fmt="YOLO", names: dict=None, name_error="warn") -> list:
    ...
    Args:
        ...
        fmt: dataset format: YOLO(default), labelme, the other custom fmt...
            ...
            with open(lb_file, encoding="utf-8") as f:
                if fmt == "YOLO":
                    ...
                    for item in json.load(f)["shapes"]:
                        if (label := names.get(item["label"])) is not None:
                            lb.append([label] + [
                                np.clip(v / s, 0, 1) for xy in item["points"] for v, s in zip(xy, shape[::-1])])
